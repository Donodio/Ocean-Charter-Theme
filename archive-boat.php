<?php
/**
 * The template for displaying boat archives (BBC Search Results)
 * Ocean Charter — mirrors the Fleet Page design exactly.
 *
...
<div class="oc-container">
	...
	<!-- Header — identical to Fleet Page -->
	<div class="fl-header">
	...
	<!-- Glass Filter Bar — identical to Fleet Page -->
	...
	<?php else : ?>
		<div class="fl-no-results" style="grid-column:1/-1;">
			<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="rgba(217,178,48,0.4)" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
			<h3><?php esc_html_e( 'No vessels found', 'ocean-charter' ); ?></h3>
			<p><?php esc_html_e( 'Try adjusting your search criteria.', 'ocean-charter' ); ?></p>
			<a href="<?php echo esc_url( get_post_type_archive_link( 'boat' ) ); ?>" class="btn-primary"><?php esc_html_e( 'View All Vessels', 'ocean-charter' ); ?></a>
		</div>
	<?php endif; ?>
	...
	<div id="fl-js-no-results" style="display:none;grid-column:1/-1;text-align:center;padding:60px 0;">
		<p style="color:var(--text-muted)"><?php esc_html_e( 'No vessels match your filters.', 'ocean-charter' ); ?></p>
		<button type="button" id="fl-js-clear-btn" style="margin-top:12px;padding:8px 20px;background:transparent;border:1px solid var(--primary);color:var(--primary);border-radius:4px;cursor:pointer;font-size:14px;"><?php esc_html_e( 'Clear Filters', 'ocean-charter' ); ?></button>
	</div>
...
(function () {
  ...
  selects.forEach(function (sel) {
    ...
    var urlVal = (params.get(key) || (key === 'type' && params.get('boat_type')) || '').toLowerCase().trim();

    if (key === 'location' && urlVal) {
      var matchedOpt = Array.from(sel.options).find(function (o) {
        return o.value && (o.value.includes(urlVal) || urlVal.includes(o.value));
      });
  ...
  function applyFilters() {
    ...
    if (badge && badgeText) {
      if (hasFilter) {
        ...
        if (fPrice)    parts.push('$' + fPrice.replace('-', '–'));
